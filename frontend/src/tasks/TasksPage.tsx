...
export function TasksPage() {
    ...
    // The active list is its own bounded scroll area; auto-load the next page as its sentinel nears
    // the bottom
    ...
    // Show fresh search results from the top rather than wherever the previous list was scrolled.
    useEffect(() => {
    ...
    return (
        <>
            ...
            {tasks.status === "loading" && (
                <p className="muted state-row">Loading your tasks…</p>
            )}
            ...
            {tasks.tasks.length > 0 && (
                <ul className="task-list scroll-list" ref={listRef}>
