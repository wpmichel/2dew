...
export function useCompletedTasks(
    ...
    // Load the first page on mount and whenever the active list signals a change.
    ...
    useEffect(() => {
        const handle = setTimeout(() => {
            ...
            (async () => {
                try {
                    const page = await client.listCompletedTasks({
                        limit: PAGE_SIZE,
                    });
    ...
    const markIncomplete = useCallback(
        async (task: TaskResponse) => {
            ...
            try {
                ...
                setTasks((prev) =>
                    insertAt(prev, Math.min(index, prev.length), snapshot),
                );
                setError(messageOf(err));
            } finally {
